...
export default function ReportForm({ onGenerateReport, isGenerating, initialData }: ReportFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-6 shadow-sm">
      {/* Date Input */}
      <div className="mb-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            onClick={addActivity}
            disabled={isGenerating}
            className="px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            + Add Activity
          </button>
        </div>
          ...
          {activities.map((activity, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              <input
                ...
                disabled={isGenerating}
                className="flex-1 px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              />
              {activities.length > 1 && (
                <button
                  type="button"
                  ...
                  disabled={isGenerating}
                  className="p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      ...
      <button
        type="submit"
        disabled={isGenerating}
        className="w-full px-4 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
      >
        {isGenerating ? (
